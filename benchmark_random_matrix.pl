...
sub solve_LP {
    ...
    my $tableau_object =   $model eq 'float'    ? Model::Float->new($tableau)
    ...
    $tableau_object->convert_natural_number_tableau_to_fractional_object_tableau if ( $model eq 'rational' );
...
sub random_float_matrix {
    ...
    for my $i (0..$rows-1) {
        for my $j (0..$columns-1) {
            $matrix->[$i]->[$j] = $natural_numbers == 0 ? rand : int(10*rand);
        }
    }
