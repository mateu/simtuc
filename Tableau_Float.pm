...
sub pivot {
    ...
    my $scale =
      $_one / ( $self->{_tableau}->[$_pivot_row]->[$_pivot_column] );
    for my $j ( 0 .. $self->{_number_of_columns} ) {
        $self->{_tableau}->[$_pivot_row]->[$j] =
          $self->{_tableau}->[$_pivot_row]->[$j] * ($scale);
    }
    ...
    for my $i ( 0 .. $self->{_number_of_rows} ) {
        if ( $i != $_pivot_row ) {
            ...
            my $neg_a_ic =
              $self->{_tableau}->[$i]->[$_pivot_column] * ($neg_one);
            for my $j ( 0 .. $self->{_number_of_columns} ) {
                $self->{_tableau}->[$i]->[$j] =
                  $self->{_tableau}->[$i]->[$j] + (
                    $neg_a_ic * ( $self->{_tableau}->[$_pivot_row]->[$j] ) );
            }
            $self->{_tableau}->[$i]->[$_pivot_column] = $neg_a_ic * ($scale);
        }
    }
...
sub set_tableau {
    ...
    my $html_tableau =
      return_html_tableau_from_file( $session, $is_example, $tmp_path,
        $examples_path );
...
sub current_solution_as_HTML {
    ...
    for my $i ( 0 .. $#y ) {
        if ( $y[$i]->{'descriptive'} ne $EMPTY_STRING ) {
            $primal_solution .=
                $y[$i]->{'descriptive'} . " = "
              . $self->{_tableau}->[$i]->[ $self->{_number_of_columns} ]
               . "<br />\n";
        }
        else {
            $primal_solution .=
    ...
    for my $j ( 0 .. $#u ) {
        my $var =
          $self->{_tableau}->[ $self->{_number_of_rows} ]->[$j] * ($neg_one);
        if ( $u[$j]->{'descriptive'} ne $EMPTY_STRING ) {
            $dual_solution .=
              $u[$j]->{'descriptive'} . " = " . $var . "<br />\n";
        }
        else {
            $dual_solution .=
              $u[$j]->{'generic'} . " = " . $var . "<br />\n";
        ...
        foreach my $j ( 0 .. $self->{_number_of_columns} ) {
            ...
            if ($pivot_links != 0) {
                    ...
                    && ( $pivot_links != 0 ) )
                {
                    ...
                    $a->push_content( $self->{_tableau}->[$i]->[$j] );
                ...
                if ( $i == $self->{_number_of_rows} ) {
                    ...
                    $html_tableau->setCellClass( $i + 1, $j + 1, 'coefficient_cell' );
                }
            ...
            # numbers currently arrise from the examle input form.
                ...
                $html_tableau->setCell( $i + 1, $j + 1, $self->{_tableau}->[$i]->[$j] );
            }
        }
    }
    $html_tableau->setLastColClass('last_column') if $pivot_links != 0;
...
sub tucker_tableau_as_HTML {
    ...
    for my $j ( 2 .. $n_cols - 2 ) {
        my $x =
            $self->{_x_variables}->[ $j - 2 ]->{'descriptive'} ne $EMPTY_STRING
        ...
        my $u =
            $self->{_u_variables}->[ $j - 2 ]->{'descriptive'} ne $EMPTY_STRING
    ...
    for my $i ( 2 .. $n_rows - 2 ) {
        ...
        my $v =
            $self->{_v_variables}->[ $i - 2 ]->{'descriptive'} ne $EMPTY_STRING
        ...
        my $y =
            $self->{_y_variables}->[ $i - 2 ]->{'descriptive'} ne $EMPTY_STRING
...
#    my $title         = $page_tree->look_down( class  => 'title' );
...
sub write_to_tied_title_hash {
    ...
    tie my %titles, 'Tie::File::AsHash', $titles_file, split => ' => '
      or die
"ERROR: Problem tying %titles: does the titles file, $titles_file have a problem? ERROR: $!";
...
sub get_example_list_as_HTML {
    ...
    foreach my $example ( sort { $hash_ref->{$a} cmp $hash_ref->{$b} }
        keys %{$hash_ref} )
    {
...
# check basement row for having non-positive entries which would => optimal when in phase 2.
    my $optimal_flag = 1;
    for my $j ( 0 .. $self->{_number_of_columns}-1 ) {
        if ( $self->{_tableau}->[ $self->{_number_of_rows} ]->[$j] >
            0 )
        {
            $optimal_flag = 0;
        }
...
# This would only apply then in the optimal case, i.e. all entries non-positive.
    for my $col_num ( 0 .. $self->{_number_of_columns} - 1 ) {
        if ( $self->{_tableau}->[ $self->{_number_of_rows} ]->[$col_num] > 0 )
        {
...
sub get_bland_simplex_pivot {
    ...
    my @simplex_pivot_columns;
    @simplex_pivot_columns = @{ $self->get_simplex_pivot_columns }
      if $self->get_simplex_pivot_columns();
    ...
    my @bland_column = $self->min_index( \@column_bland_numbers );
...
# To be a Simplex pivot we must not consider negative entries
    ...
    my @min_ratio_rows_index = $self->min_index($ratios_ref);

    # Apply Bland Number Ranking to Rows that tie for minimum ratio.
    my @bland_numbers;
    foreach my $min_ratio_row_index (@min_ratio_rows_index) {
        my $bland_number =
          $self->get_bland_number_for( 'y', $min_ratio_row_index );
        push @bland_numbers, $bland_number;
    }
    my @min_bland_number_index = $self->min_index( \@bland_numbers );
    my $min_bland_number_index = $min_bland_number_index[0];
    my $bland_pivot_row        =
      $ratio_rows[ $min_ratio_rows_index[$min_bland_number_index] ];
